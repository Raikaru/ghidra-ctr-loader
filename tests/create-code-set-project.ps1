...
try {
    ...
    $raw = docker exec $Container bash -lc "/opt/ghidra/support/analyzeHeadless $containerProjectRoot $containerProjectName -import $containerWork/input/input.code -processor 'ARM:LE:32:v7' -cspec default -scriptPath $containerWork/scripts -preScript MapCtrCodeSet.java $containerWork/input/manifest.structure.json" 2>&1
